...
class ComputeOpticaldepth:
    ...
    def __init__(self, Wizard):
        ...
        self.inputfunc          = InputFunctions(fileparams=self.specparams, header=self.specparams['Header'])
    ...
    def MakeAllOpticaldepth(self, projected_los):
        ...
        # Pixel properties
        pixel_kms = projection["Projection"]["pixel_kms"]["Value"]  # One pixel in km/s
        pixel = self.to_physical(projection["Projection"]["pixel"]).in_cgs()  # Size of pixel in cm
        sight_kms = projection["SightInfo"]["sightkms"]["Value"]
        ...
        if self.ThermEff:
            spectra['ThermalEffectsOff'] = self.WrapSpectra(
                Ions,
                projectionIW,
                sightparams,
                vel_mod=False,
        ...
        if self.PecVelEff:
            spectra['PeculiarVelocitiesOff'] = self.WrapSpectra(
                Ions,
                projectionIW,
                sightparams,
                vel_mod=True,
        ...
        if self.ThermEff and self.PecVelEff:
            spectra['ThermalPecVelOff'] = self.WrapSpectra(
                Ions,
                projectionIW,
                sightparams,
                vel_mod=True,
        ...
        if DoSimIons:
            ...
            spectra["SimIons"] = self.WrapSpectra(
                SimElmsIons,
                projectionSIW,
                sightparams,
                vel_mod=False,
            ...
            if self.ThermEff:
                spectra["SimIons_ThermalEffectsOff"] = self.WrapSpectra(
                    SimElmsIons,
                    projectionSIW,
                    sightparams,
                    vel_mod=False,
            ...
            if self.PecVelEff:
                spectra['SimIons_PeculiarVelocitiesOff'] = self.WrapSpectra(
                    SimElmsIons,
                    projectionSIW,
                    sightparams,
                    vel_mod=True,
            ...
            if self.ThermEff and self.PecVelEff:
                spectra['SimIons_ThermalPecVelOff'] = self.WrapSpectra(
                    SimElmsIons,
                    projectionSIW,
                    sightparams,
                    vel_mod=True,
    ...
    def MakeOpticaldepth(self, sightparams={'sight_kms': 0.0, 'vel_kms': [0.0], 'pixel_kms': 1.0, 'pixel': 1.0},
        ...
        dunit = self.SetUnit(vardescription="Total ion column density", aFact=0.0, hFact=0.0)
        total_column_density = {'Value': total_column_density, "Info": dunit}

        # Compute b-parameter (velocity dispersion)
        bions_kms = np.sqrt(2 * self.constants["kB"] * Tions / weight).to("km/s")
        ...
        spectrum = lines.gaussian(
            column_densities=ioncolumns,
            b_kms=bions_kms,
...
#        print("lines out:",tau.units)
        ...
        hydrogen_densities = spectrum.get('optical_depth_hydrogen_densities', None)
        metallicities = spectrum.get('optical_depth_metallicities', None)
 ...
 #       print("convol out",tau.units)
        ...
        if hydrogen_densities is not None:
            output['HydrogenDensities'] = hydrogen_densities
        if metallicities is not None:
            output['Metallicities'] = metallicities
