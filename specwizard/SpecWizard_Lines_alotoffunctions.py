...
class Lines:
    ...
    def __init__(self, v_kms =0.0, box_kms=-1.0, constants=constants, verbose=False, 
        ...
        self.CG  = {'Spline':CGspline, 'table_x':x, 'table_y': CG}
        v_max = 30
        v_min = 0
        self.nfine      = 1000000
    ...
    def UniversalErf(self,b_kms=10,v0_kms=0.0 ):
        ...
        phi[phi==-1] = 0
        phi     = np.roll(phi,1)
        phi = np.roll(phi,v0_displacement-1)
    ...
    def EvenBetter(self,b_kms=10.0, v0_kms=0.0):
        
        u      = self.PeriodicVelocity(v0_kms=v0_kms) / b_kms
        
        #x        = np.arange(0, u.max(), 1e-2)
        x        = np.linspace(0,u.max(),15000)
        CG       = 0.5 * (1.+erf(x))
        x        = np.concatenate((-np.flip(x[1:]), x))
        CG       = np.concatenate((1-np.flip(CG[1:]), CG))
#        print(u.min(),u.max())
#        scaled_array = (30/abs(u.min()))*u
        print(len(u),len(x))
        indx = np.array([np.argmin(np.abs(x-i)) for i in u])
        print("Done")
        cphi = CG[indx]
        phi    = (np.roll(cphi, -1) - cphi) 
        phi[phi==-1] = 0
    ...
    def OldGauss(self, b_kms=10.0, v0_kms=0.0):
        ''' return line-profile function for Gaussian profile, Equation 3
            This version uses interpolation from the pre-computed integral of the Gaussian, to ensure
            correct normalization independent of pixel size
        input:
    ...
    def Gauss(self, b_kms=10.0, v0_kms=0.0):
        ''' return line-profile function for Gaussian profile, Equation 3
            This version uses interpolation from the pre-computed integral of the Gaussian, to ensure
            correct normalization independent of pixel size
        input:
        ...
        phi    /=  self.pix_kms
        phi     = np.roll(phi,1)
        return phi/1e5 #Cphi#phi/1e5                                   # convert to units of [s/cm]
